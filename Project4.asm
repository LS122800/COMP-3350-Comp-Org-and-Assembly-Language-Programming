...
.code
    ...
    Decrypt proc
        ...
        L:
            ...
            mov bl, key[edi]                ; move the key ASCII character into bl
            sub bl, 65                      ; get the order of the character ( A=0, B=1, C=2, ... , Z=25)
            sub al, bl                      ; subtract the indices to "decrypt" the character
            add al, 26                      ; add 26 in case subtraction result is a negative value
            div d                           ; use divide to get "mod 26". AX will be divided, quotient in AL and reminder in AH. 
            add ah, 65                      ; add 65 to convert reminder back from order to the proper ASCII value
            mov output[esi], ah             ; write ASCII value to output
            inc esi			
            inc edi
            cmp edi, lengthof key 
            jne Next
            mov edi, 0                      ; if key index reached the end, reset the key index
        Next:
            loop L
            ret
    Decrypt endp
    ...
    Encrypt proc
        ...
        L:
            ...
            add al, bl                      ; add the indices to encrypt the character
            div d                           ; divide so we can "mod 26"
            add ah, 65                      ; add 65 to convert from order back to ASCII value
            mov output[esi], ah             ; write reminder into the output
            inc esi
            inc edi
            cmp edi, lengthof key           ; if reached end of key
            jne Next
